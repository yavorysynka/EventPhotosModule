...
namespace RK\EventPhotosModule\ContentType\Base;
...
abstract class AbstractItemList extends \Content_AbstractContentType implements ContainerAwareInterface
{
    ...
    {
        return $this->container->get('translator.default')->__('RKEventPhotosModule list view');
    }
    ...
    {
        return $this->container->get('translator.default')->__('Display list of RKEventPhotosModule objects.');
    }
    ...
    {
        $controllerHelper = $this->container->get('rk_eventphotos_module.controller_helper');
        ...
        $featureActivationHelper = $this->container->get('rk_eventphotos_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            ...
            $categoryHelper = $this->container->get('rk_eventphotos_module.category_helper');
    
            // fetch category properties
            $this->catRegistries = [];
            $this->catProperties = [];
            if (in_array($this->objectType, $this->categorisableObjectTypes)) {
                $entityFactory = $this->container->get('rk_eventphotos_module.entity_factory');
    ...
    {
        $repository = $this->container->get('rk_eventphotos_module.entity_factory')->getRepository($this->objectType);
        $permissionApi = $this->container->get('zikula_permissions_module.api.permission');
    
        // create query
        $orderBy = $this->container->get('rk_eventphotos_module.model_helper')->resolveSortParameter($this->objectType, $this->sorting);
        ...
        $templateParameters = $this->container->get('rk_eventphotos_module.controller_helper')->addTemplateParameters($this->objectType, $templateParameters, 'contentType', []);
    ...
    public function startEditing()
    {
        ...
        $this->view->toplevelmodule = 'RKEventPhotosModule';
    
        // ensure our custom plugins are loaded
        array_push($this->view->plugins_dir, 'modules/RK/EventPhotosModule/Resources/views/plugins');
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            ...
            $this->view->assign('categories', $categories)
                       ->assign('categoryHelper', $this->container->get('rk_eventphotos_module.category_helper'));
        }
