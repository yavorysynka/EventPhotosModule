...
function rKEventPhotosCapitaliseFirstLetter(string) {
 ...
 */
function rKEventPhotosInitQuickNavigation() {
    var quickNavForm;
    var objectType;

    if (jQuery('.rkeventphotosmodule-quicknav').length < 1) {
        return;
    }

    quickNavForm = jQuery('.rkeventphotosmodule-quicknav').first();
    objectType = quickNavForm.attr('id').replace('rKEventPhotosModule', '').replace('QuickNavForm', '');

    quickNavForm.find('select').change(function (event) {
        quickNavForm.submit();
    });

    var fieldPrefix = 'rkeventphotosmodule_' + objectType.toLowerCase() + 'quicknav_';
 ...
 */
function rKEventPhotosSimpleAlert(anchorElement, title, content, alertId, cssClass) {
    var alertBox;
 ...
 */
function rKEventPhotosInitMassToggle() {
    if (jQuery('.rkeventphotos-mass-toggle').length > 0) {
        jQuery('.rkeventphotos-mass-toggle').unbind('click').click(function (event) {
            jQuery('.rkeventphotos-toggle-checkbox').prop('checked', jQuery(this).prop('checked'));
        });
    }
}
 ...
 */
function rKEventPhotosInitItemActions(context) {
    if (context == 'display') {
    ...
    if (context == 'view') {
        ...
        containerSelector = '';
        if (context == 'view') {
            containerSelector = '.rkeventphotosmodule-view';
 ...
 */
function rKEventPhotosInitInlineWindow(containerElem) {
    var newWindowId;
    var modalTitle;
 ...
 */
function rKEventPhotosInitQuickViewModals() {
    jQuery('.rkeventphotos-inline-window').each(function (index) {
        rKEventPhotosInitInlineWindow(jQuery(this));
    });
}
...
jQuery(document).ready(function () {
    ...
    isViewPage = jQuery('.rkeventphotosmodule-view').length > 0;
    isDisplayPage = jQuery('.rkeventphotosmodule-display').length > 0;

    rKEventPhotosInitImageViewer();

    if (isViewPage) {
        rKEventPhotosInitQuickNavigation();
        rKEventPhotosInitMassToggle();
        rKEventPhotosInitItemActions('view');
    } else if (isDisplayPage) {
        rKEventPhotosInitItemActions('display');
    }

    rKEventPhotosInitQuickViewModals();
});
