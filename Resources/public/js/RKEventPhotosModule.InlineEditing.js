 ...
 */
function rKEventPhotosCreateInlineEditingWindowInstance(containerElem) {
    var newWindowId;
 ...
 */
function rKEventPhotosInitInlineEditingWindow(objectType, idPrefix, containerId, inputType) {
    ...
    jQuery.each(rKEventPhotosInlineEditHandlers, function (key, editHandler) {
        ...
        editHandler.windowInstanceId = rKEventPhotosCreateInlineEditingWindowInstance(jQuery('#' + containerId));
    });
    ...
    newEditHandler = {
        ...
        inputType: inputType,
        windowInstanceId: rKEventPhotosCreateInlineEditingWindowInstance(jQuery('#' + containerId))
    };

    // add it to the list of edit handlers
    rKEventPhotosInlineEditHandlers.push(newEditHandler);
}

/**
 * Creates a link for editing an existing item using inline editing.
 */
function rKEventPhotosCreateInlineEditLink(objectType, idPrefix, elemPrefix, itemId) {
 ...
 */
function rKEventPhotosInitInlineEditLink(objectType, idPrefix, elemPrefix, itemId, inputType) {
    jQuery('#' + elemPrefix + 'Edit').click(function (event) {
        event.preventDefault();
        rKEventPhotosInitInlineEditingWindow(objectType, idPrefix, idPrefix + 'Reference_' + itemId + 'Edit');
    });
}
 ...
 */
function rKEventPhotosDetermineInputReference(objectType, alias, idPrefix, inputType, targetWindow) {
    var inputPrefix, inputIdentifier, inputField;

    // determine reference to input element
    inputPrefix = targetWindow.jQuery('.rkeventphotos-edit-form').first().attr('name');
    inputField = null;
 ...
 */
function rKEventPhotosInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl) {
    var inputReference, createButtonId, createButton, itemIds, itemIdsArr;

    inputReference = rKEventPhotosDetermineInputReference(objectType, alias, idPrefix, inputType, window);
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a />', {
            ...
            class: 'btn btn-default rkeventphotosmodule-inline-button'
        }).append(
    ...
    createButton.click(function (event) {
        event.preventDefault();
        rKEventPhotosInitInlineEditingWindow(objectType, idPrefix, createButtonId, inputType);
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                rKEventPhotosInitInlineEditingWindow(objectType, idPrefix, elemPrefix, inputType);
            });
        }
    });
}
 ...
 */
function rKEventPhotosCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm) {
    ...
    jQuery.each(window.parent.rKEventPhotosInlineEditHandlers, function (key, editHandler) {
        ...
        inputReference = rKEventPhotosDetermineInputReference(editHandler.objectType, editHandler.alias, idPrefix, inputType, window.parent);
        if (null === inputReference || null === inputReference.field) {
            return;
        }
        ...
        window.parent.rKEventPhotosSimpleAlert(anchorElement, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
